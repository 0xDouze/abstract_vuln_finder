...
TEST (Func, DefaultConstructor){
  ...
  EXPECT_EQ(std::list<Arc>().empty(), func.get_func_calls().empty());

  std::list<std::any> test;
  test.push_back("coucou");
  func.add_arg("coucou");
  //EXPECT_STREQ(std::any_cast<char*>(*(test.begin())), std::any_cast<char*>(*(func.get_args().begin())));
  std::cout << *test.begin()->type().name() << std::endl;
  std::cout << *test.begin()->type().name() << std::endl;
  EXPECT_EQ(*test.begin()->type().name(), *(func.get_args().begin())->type().name());
  Arc arc;
  arc.id = 1;
  func.add_func_call(arc);
  std::list<Arc> tamere;
