...
std::shared_ptr<Node> TransformToCFG::parse_instructions(std::shared_ptr<Node> node, const std::list<llvm::Instruction*>::iterator begin, std::list<llvm::Instruction*>::iterator current, const std::list<llvm::Instruction*>::iterator end)
{
  if (current == end)
    return nullptr;
...
std::shared_ptr<Node> TransformToCFG::convert_function_to_node(llvm::Function *func)
{
  llvm::SMDiagnostic err;
  std::shared_ptr<Node> start = std::make_shared<Node>();
  std::list<llvm::Instruction*> inst;
